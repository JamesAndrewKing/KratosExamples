...
class LegendreOutputMap:
    """Reconstruct selected current probe displacements from the SSM geometry."""

    def __init__(self, data):
        if int(data.get("format_version", 1)) < 2:
            raise ValueError("The ROM export does not contain a delay-space output map.")
        # Precompose coordinatesReconstruction with W_delay. The exported
        # reconstruction matrix was generated by SSMLearn's helper.
        self.coefficients = _matrix_multiply(
            data["mpc_output_reconstruction"], data["geometry_coefficients"])
        self.eta_exponents = data["geometry_eta_exponents"]
        self.eta_center = _vector(data["geometry_eta_center"])
        self.eta_scale = _vector(data["geometry_eta_scale"])
        self.feature_limit = float(data["geometry_feature_limit"])
        self.target = _vector(data["mpc_output_target"])
        self.weights = _vector(data["mpc_output_weights"])
        self.names = data["mpc_output_names"]
        if not (len(self.coefficients) == len(self.target) == len(self.weights)):
            raise ValueError("MPC output map, target, and weights have incompatible sizes.")

    def evaluate(self, eta):
        value, _ = self.evaluate_with_jacobian(eta)
        return value

    def evaluate_with_jacobian(self, eta):
        basis, derivatives = _eta_legendre_basis(
            eta, self.eta_exponents, self.eta_center,
            self.eta_scale, self.feature_limit)
        value = [_dot(row, basis) for row in self.coefficients]
        jacobian = [[_dot(row, derivatives[j]) for j in range(len(eta))]
                    for row in self.coefficients]
        return value, jacobian

    def cost(self, eta):
        value = self.evaluate(eta)
        return sum(self.weights[i] * (value[i] - self.target[i]) ** 2
                   for i in range(len(value)))

    def cost_gradient(self, eta):
        value, jacobian = self.evaluate_with_jacobian(eta)
        error = [value[i] - self.target[i] for i in range(len(value))]
        cost = sum(self.weights[i] * error[i] ** 2 for i in range(len(value)))
        gradient = [2.0 * sum(
            self.weights[i] * error[i] * jacobian[i][j] for i in range(len(value)))
            for j in range(len(eta))]
...
class ReducedMpc:
    ...
    def __init__(self, rom, output, target, control_bound, control_interval,
                 prediction_horizon, max_control_increment, control_weight,
                 increment_weight, terminal_weight,
    ...
    def _objective(self, eta0, decision, previous_control):
        ...
        for step in range(self.steps):
            ...
            cost += self.dt * (
                self.output.cost(eta)
                + self.control_weight * (control / self.bound) ** 2
                + self.increment_weight * ((control - old_control) / self.bound) ** 2
            )
            eta = self.rom.step(eta, control, self.dt)
            old_control = control
        return cost + self.terminal_weight * self.output.cost(eta)
    ...
    def _objective_gradient(self, eta0, decision, previous_control):
        ...
        for step in range(self.steps):
            block = min(self.blocks - 1, step * self.blocks // self.steps)
            control = decision[block]
            output_cost, output_gradient = self.output.cost_gradient(eta)
            cost += self.dt * (
                output_cost
                + self.control_weight * (control / self.bound) ** 2
                + self.increment_weight * ((control - old_control) / self.bound) ** 2
            ...
            for j in range(self.blocks):
                gradient[j] += self.dt * sum(
                    output_gradient[i] * sensitivity[i][j] for i in range(len(eta)))
            gradient[block] += (
                2 * self.control_weight * self.dt * control / self.bound ** 2)
            rate = control - old_control
            gradient[block] += (
                2 * self.increment_weight * self.dt * rate / self.bound ** 2)
        ...
        output_cost, output_gradient = self.output.cost_gradient(eta)
        cost += self.terminal_weight * output_cost
        for j in range(self.blocks):
...
class RomMpcController:
    ...
    def __init__(self, model, settings):
        ...
        self.output = LegendreOutputMap(data)
        ...
        self.mpc = ReducedMpc(
            ...
            float(data["mpc_control_weight"]),
            float(data["mpc_increment_weight"]),
            float(data["mpc_terminal_weight"]),
        ...
        self.writer.writerow([
            ...
            "control_increment", "next_control_time", "output_error"
        ])
...
def _eta_legendre_basis(eta, exponents, center, scale, feature_limit):
    coordinates = [
        _clip((eta[i] - center[i]) / scale[i], feature_limit)
        for i in range(len(eta))
    ]
    data = [_legendre_values_and_derivatives(
        coordinates[i], max(row[i] for row in exponents))
        for i in range(len(eta))]
    basis = []
    derivatives = [[] for _ in eta]
    for exponent in exponents:
        factors = [data[j][0][degree] for j, degree in enumerate(exponent)]
        basis.append(math.prod(factors))
        for j, degree in enumerate(exponent):
            others = math.prod(factors[k] for k in range(len(factors)) if k != j)
            inside = abs(coordinates[j]) < feature_limit
