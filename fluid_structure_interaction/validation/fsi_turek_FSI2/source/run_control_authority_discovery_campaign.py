"""Run a minimal FSI2 scalar-control authority-discovery campaign.

This campaign is intentionally small. It is meant to discover which scalar
control amplitudes can move the developed passive limit cycle, not to generate
a dense identification library.

The scalar control convention is the same as the previous control campaigns:
...
DEFAULT_CAMPAIGN_LABEL = "fsi_control_authority_discovery_t40_u20"
DEFAULT_U_MAX = 2.0
DEFAULT_U_KICK = 0.4
...
DEFAULT_INCLUDE_T5_STRONG_CASES = False
DEFAULT_MAX_REASONABLE_TIP_DISPLACEMENT = 0.5

INTERVENTION_TARGETS = [-0.5, 0.5, -1.0, 1.0, -1.5, 1.5, -2.0, 2.0]
OPTIONAL_T5_TARGETS = [-2.0, 2.0]
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run or audit the minimal FSI2 scalar-control authority-discovery campaign."
    )
...
def read_settings():
    return {
        "campaign_label": os.environ.get("KRATOS_FSI_AUTH_LABEL", DEFAULT_CAMPAIGN_LABEL),
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "u_max": read_float("KRATOS_FSI_AUTH_U_MAX", DEFAULT_U_MAX),
        "u_kick": read_float("KRATOS_FSI_AUTH_U_KICK", DEFAULT_U_KICK),
        "end_time": read_float("KRATOS_FSI_AUTH_END_TIME", DEFAULT_END_TIME),
        "t_kick_end": read_float("KRATOS_FSI_AUTH_T_KICK_END", DEFAULT_T_KICK_END),
        "t_intervention": read_float("KRATOS_FSI_AUTH_T_INTERVENTION", DEFAULT_T_INTERVENTION),
        "ramp_duration": read_float("KRATOS_FSI_AUTH_RAMP_DURATION", DEFAULT_RAMP_DURATION),
        "output_dt": read_float("KRATOS_FSI_AUTH_OUTPUT_INTERVAL", DEFAULT_OUTPUT_INTERVAL),
        "input_dt": read_float("KRATOS_FSI_AUTH_SIGNAL_DT", DEFAULT_SIGNAL_DT),
        "write_paraview": read_bool("KRATOS_FSI_AUTH_WRITE_PARAVIEW", DEFAULT_WRITE_PARAVIEW),
        "include_t5_strong_cases": read_bool(
            "KRATOS_FSI_AUTH_INCLUDE_T5_STRONG_CASES",
            DEFAULT_INCLUDE_T5_STRONG_CASES,
        ),
        "max_reasonable_tip_displacement": read_float(
            "KRATOS_FSI_AUTH_MAX_REASONABLE_TIP_DISPLACEMENT",
...
def build_case_library(settings):
    cases = [
        ...
        make_kick_then_passive_case(settings),
    ]
    for target in INTERVENTION_TARGETS:
        cases.append(make_kick_then_intervention_case(settings, target, settings["ramp_duration"]))
    if settings["include_t5_strong_cases"]:
        for target in OPTIONAL_T5_TARGETS:
            cases.append(make_kick_then_intervention_case(settings, target, 5.0))
    return cases
...
def collect_tip_metrics(run_directory, settings):
    ...
    if p2p_before > 1e-14:
        metrics["tip_y_peak_to_peak_relative_change_after_before"] = (p2p_after - p2p_before) / p2p_before

    relative_changes = [
        value for value in (
            metrics["rms_tip_y_relative_change_after_before"],
            metrics["max_abs_tip_y_relative_change_after_before"],
            metrics["tip_y_peak_to_peak_relative_change_after_before"],
        )
        if value != ""
    ]
    if relative_changes:
        metrics["tip_authority_score"] = max(abs(value) for value in relative_changes)
...
def audit_campaign(campaign_directory, manifest, results):
    ...
    if errors:
        ...
        for error in errors:
            print(f"  ERROR: {error}")
        raise RuntimeError("Authority-discovery campaign audit failed.")
...
def make_manifest(settings, cases):
    return {
        ...
        "purpose": "minimal scalar-control authority threshold discovery",
        "base_campaign": "fsi_control_parametric_manifold_t40_u04",
        ...
        "include_t5_strong_cases": settings["include_t5_strong_cases"],
        "optional_t5_targets": OPTIONAL_T5_TARGETS if settings["include_t5_strong_cases"] else [],
        "end_time": settings["end_time"],
        "output_dt": settings["output_dt"],
        "input_dt": settings["input_dt"],
        ...
        "main_diagnostic": (
            "tip_authority_score = max absolute relative change in RMS, max-abs, "
            "and peak-to-peak tip displacement between [15,20] and [30,40]"
        ),
...
def select_cases(cases):
    labels = os.environ.get("KRATOS_FSI_AUTH_CASE_LABELS")
    index = os.environ.get("KRATOS_FSI_AUTH_CASE_INDEX")
    limit = os.environ.get("KRATOS_FSI_AUTH_LIMIT")
    if labels:
    ...
    if index is not None:
        ...
        if index < 0 or index >= len(cases):
            raise IndexError(f"KRATOS_FSI_AUTH_CASE_INDEX={index} outside 0..{len(cases)-1}.")
